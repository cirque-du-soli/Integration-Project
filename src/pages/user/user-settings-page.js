...
const UserSettings = () => {
    ...
    useEffect(() => {
        const fetchUserProfilePicture = async () => {
            try {
                ...
                const response = await axios.get(`${baseUrl}/settings/profilepicture`, { responseType: 'blob', headers: { accessToken: localStorage.getItem("accessToken") } });
    ...
    useEffect(() => {
        const fetchUserSettings = async () => {
            try {
                const response = await axios.get(`${baseUrl}/settings/`, { headers: { accessToken: localStorage.getItem("accessToken") } });
    ...
    const handleFileChange = async (event) => {
        ...
        if (file) {
            try {
                const response = await axios.patch(`${baseUrl}/settings/profilepicture`, { profilePic: file }, {
                    headers: {
    ...
    return (
        <div>
            <Navbar />
            <CenteredBox>
                <Box component="form" sx={{ '& .MuiTextField-root': { m: 1, width: '25ch', display: "block" } }} noValidate autoComplete="off">
                    <div>
                        <section>
                        ...
                        <section>
                            ...
                            >
                                <img src={userProfilePictureUrl} alt="User Profile" style={{ width: "100%", height: "100%" }} />
                                <ImageOverlay>
                                    <UploadButton color="primary" component="label" style={{ visibility: isHovering ? 'visible' : 'hidden' }}>
                                        Upload
                                        <input type="file" id="profile-image-upload" name="profileImage" accept="image/*" onChange={handleFileChange} style={{ display: 'none' }} />
                                    </UploadButton>
                                </ImageOverlay>
                            </ImageWrapper>
                            ...
                            <StyledModal open={modalIsOpen} onClose={() => setModalIsOpen(false)} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
                                <ModalBox sx={{
                                    ...
                                    <Button
                                        ...
                                        disabled={ // Button is disabled if any of these conditions are true:
