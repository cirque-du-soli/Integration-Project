...
const AdminNavTabs = ({ props }) => {
    ...
    return (
        <div className="container mx-auto px-4 py-4 bg-base-100 rounded-xl">
            ...
            <div className='p-5 bg-base-300 mx-3'>
                ...
                {currentActiveTab === 'statistics' && (
                    <><div className="bg-base-100 px-2 rounded-lg" >
                        <div className="bg-base-100 tabs tabs-lifted text-sm md:text-md lg:text-lg mb-2 md:mb-3 lg:mb-4 p-5 m-5">
                            ...
                            <a className={`tab tab-disabled ${statsTab === 'messages' ? 'tab-active [--tab-bg:#3ffc6e] [--tab-border-color:#00db37] text-black' : '[--tab-border-color:#00db37]'}`} /* onClick={() => toggleStatsTab('messages')} */>Messages</a>
                            <a className={`tab tab-disabled ${statsTab === 'mosaics' ? 'tab-active [--tab-bg:#3ffc6e] [--tab-border-color:#00db37] text-black' : '[--tab-border-color:#00db37]'}`} /* onClick={() => toggleStatsTab('mosaics')} */>Mosaics</a>
                        </div>
                        <div className={`content max-w-[1000px] mx-auto ${currentActiveTab === 'statistics' ? 'block' : 'hidden'}`}>
